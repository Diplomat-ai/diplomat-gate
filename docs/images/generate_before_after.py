...
def svg_root() -> ET.Element:
    ...
    ET.SubElement(
        root, "title", {"id": "title"}
    ).text = "diplomat-gate before and after: same agent, different outcome"
...
def draw_panel_label(parent, x, y, label_text, color):
    ...
    text(parent, x, y, label_text, size=11, weight="700", color=color, anchor="start")


def draw_terminal(parent, x, y, w, h, lines):
    """A stylized terminal window with traffic-light dots and content."""
    # Window body
    rounded_rect(parent, x, y, w, h, fill=TERMINAL_BG, stroke=TERMINAL_BORDER, stroke_w=1, r=8)
...
def build() -> ET.Element:
    ...
    # Panel labels
    draw_panel_label(root, PANEL_X_LEFT, PANEL_Y - 10, "WITHOUT diplomat-gate", LABEL_WITHOUT)
    draw_panel_label(root, PANEL_X_RIGHT, PANEL_Y - 10, "WITH diplomat-gate", LABEL_WITH)
    ...
    text(
        ...
        W / 2,
        H - 22,
        "Ten lines of YAML. Deterministic. Framework-agnostic. Hash-chained audit.",
        size=13,
        weight="500",
        color=TEXT_MUTED,
        anchor="middle",
