...
def svg_root() -> ET.Element:
    ...
    ET.SubElement(root, "title", {"id": "title"}).text = (
        "diplomat-gate: one enforcement layer for four agent frameworks"
    )
...
def build() -> ET.Element:
    ...
    text(root, COL_FRAMEWORK_X, 50,
         "YOUR AGENT FRAMEWORK",
         size=11, weight="600", color=TEXT_MUTED)
    text(root, COL_GATE_X + COL_GATE_W / 2, 50,
         "ENFORCEMENT LAYER",
         size=11, weight="600", color=TEXT_MUTED, anchor="middle")
    text(root, COL_VERDICT_X, 50,
    ...
    for i, fw in enumerate(FRAMEWORKS):
        ...
        rounded_rect(root, COL_FRAMEWORK_X, y, COL_FRAMEWORK_W, FW_H,
                     fill=fw["fill"], stroke=fw["stroke"])
        text(root, COL_FRAMEWORK_X + 20, y + 32,
             fw["name"], size=15, weight="600", color=TEXT_PRIMARY)
        text(root, COL_FRAMEWORK_X + 20, y + 55,
             fw["sub"], size=12, weight="400", color=TEXT_MUTED)
    ...
    rounded_rect(root, COL_GATE_X, GATE_Y, COL_GATE_W, GATE_H,
                 fill=GATE_FILL, stroke=GATE_STROKE, stroke_w=2, r=12)

    # Gate title
    text(root, COL_GATE_X + COL_GATE_W / 2, GATE_Y + 80,
         "diplomat-gate", size=22, weight="700", color="#FFFFFF",
         anchor="middle")
    ...
    for i, p in enumerate(pillars):
        text(root, COL_GATE_X + COL_GATE_W / 2,
             GATE_Y + 130 + i * 28,
             f"· {p}", size=13, weight="400", color="#E0E7FF",
             anchor="middle")

    # --- Verdict boxes (right column) ---
    for i, v in enumerate(VERDICTS):
        y = VERDICT_Y_START + i * VERDICT_Y_STEP
        rounded_rect(root, COL_VERDICT_X, y, COL_VERDICT_W, VERDICT_H,
                     fill=v["fill"], stroke=v["stroke"])
        text(root, COL_VERDICT_X + 24, y + 38,
             v["label"], size=15, weight="700", color="#FFFFFF")
