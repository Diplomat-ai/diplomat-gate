...
class StateStore:
    ...
    def record_value(
        ...
        """Record a numeric value with a timestamp, for windowed-sum policies."""
        key = self._key(policy_id, scope)
        ts = time.time() if timestamp is None else float(timestamp)
        with self._lock:
            self._data[key].values.append((ts, float(value)))

    def sum_values(self, policy_id: str, scope: str, window_seconds: float) -> float:
        """Sum of values recorded within the last ``window_seconds``."""
        key = self._key(policy_id, scope)
        cutoff = time.time() - window_seconds
        with self._lock:
            entry = self._data[key]
            entry.values = [(t, v) for t, v in entry.values if t > cutoff]
            return sum(v for _, v in entry.values)
