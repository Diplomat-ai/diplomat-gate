...
class TestStop:
    ...
    def test_blocked_domain(self):
        v = _g({"email": [{"id": "email.domain_blocklist", "blocked": ["*.banque-*.fr"]}]}).evaluate(
            {"action": "send_email", "to": "cfo@banque-marseille.fr"})
        assert v.blocked


class TestReview:
    def test_review_verdict(self):
        v = _g({"payment": [{"id": "payment.amount_limit", "max_amount": 1000, "on_fail": "REVIEW"}]}).evaluate(
            {"action": "charge_card", "amount": 5000})
...
class TestLatency:
    def test_under_5ms(self):
        g = _g({"payment": [{"id": "payment.amount_limit", "max_amount": 10000},
                ...
                "email": [{"id": "email.domain_blocklist", "blocked": ["*.x.com"]},
