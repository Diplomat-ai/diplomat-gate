...
class TestDomainBlocklist:
    def test_safe(self):
        assert (
            _g([{"id": "email.domain_blocklist", "blocked": ["*.banque-*.fr"]}])
            .evaluate({"action": "send_email", "to": "hi@startup.com"})
            .allowed
        )
    ...
    def test_list_recipients(self):
        assert (
            _g([{"id": "email.domain_blocklist", "blocked": ["*.evil.com"]}])
            .evaluate({"action": "send_email", "to": ["a@good.com", "b@evil.com"]})
            .blocked
        )
...
class TestContentScan:
    def test_clean(self):
        assert (
            _g([{"id": "email.content_scan", "patterns": ["credit_card"]}])
            .evaluate({"action": "send_email", "to": "a@b.com", "body": "Hello!"})
            .allowed
        )
    ...
    def test_api_key_in_subject(self):
        assert (
            _g([{"id": "email.content_scan", "patterns": ["api_key"]}])
            .evaluate(
                {
                    "action": "send_email",
                    "to": "a@b.com",
                    "subject": "Key: sk_live_abc123def456ghi789jkl",
                }
            )
            .blocked
        )
    ...
    def test_ssn(self):
        assert (
            _g([{"id": "email.content_scan", "patterns": ["ssn"]}])
            .evaluate({"action": "send_email", "to": "a@b.com", "body": "SSN: 123-45-6789"})
            .blocked
        )
