...
class TestLoadFromDict:
    ...
    def test_email_section(self):
        policies = load_from_dict({"email": [{"id": "email.domain_blocklist", "blocked": ["*.evil.com"]}]})
        assert len(policies) == 1
        assert isinstance(policies[0], DomainBlocklistPolicy)

    def test_policies_key(self):
        policies = load_from_dict({"policies": [
    ...
    def test_severity_propagated(self):
        policies = load_from_dict({"payment": [
    ...
    def test_velocity_window(self):
        policies = load_from_dict({"payment": [
            {"id": "payment.velocity", "max_txn": 5, "window": "30m"}
        ]})
